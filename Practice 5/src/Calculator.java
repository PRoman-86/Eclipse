/*
 Это финальная версия калькулятора, полностью доработанная и содержит
 код выполненных дополнительных и промежуточных заданий.
 */
...
public class Calculator {
	...
	JButton buttonErase=new JButton("C"); //объявляем кнопку очистки
	
	//объявляем панель
	JPanel windowContent=new JPanel();
	
	//объявляем и инициализируем текстовое поле (дисплей) длиной в 30 символов
	//объявляем и инициализируем второе поле для вывода текстовой информации длиной в 30 символов
	JTextField displayField=new JTextField(30);
	JTextField displayFieldErr=new JTextField("Калькулятор v1.2",30);
	...
	Calculator() {
		...
		windowContent.add("South",displayFieldErr);
		
		//выравниваем текст в текстовом поле вправо (собственная инициатива)
		displayField.setHorizontalAlignment(JTextField.RIGHT);
		//displayField.setEditable(false);
		displayFieldErr.setHorizontalAlignment(JTextField.CENTER);
		displayFieldErr.setEditable(false);
		...
		p2.add(buttonErase); //добавляем кнопку очистки
		p2.add(buttonPlus);
		...
		buttonErase.addActionListener(calcEngine);
